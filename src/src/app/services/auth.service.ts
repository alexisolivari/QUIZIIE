...
export class AuthService {
  user : UserInfoModel = new UserInfoModel(null,"epic_fix", null, null, null);
      ...
      if (firebase.auth().currentUser) {
        firebase.database().ref("/users/" + firebase.auth().currentUser.uid).on('value', (data) => {
            ...
            if (data.val()) {
              this.setIsAuth(true);
              let jpp = data.val();
              let previous_user = new UserInfoModel(this.user.pseudo, this.user.email, this.user.isAdmin, this.user.uid, this.user.dateaj);
              ...
              previous_user.dateaj = jpp.dateaj;
      ...
      (resolve, reject) => {
        firebase.auth().createUserWithEmailAndPassword(email, password).then(
          () => {
            ...
            this.setUserInfo(new UserInfoModel(null, email, false, uid, Date.now()));
            resolve();
          },
          (error) => {
            reject(error);
